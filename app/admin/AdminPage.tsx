          ...
          <table className="w-full text-sm table-fixed">
            ...
            <tbody>
              {filtered.map(q => {
                ...
                const displayTitle = q.title || q.question
                return (
                  <tr
                    key={q.id}
                      ...
                      {q.problemTitle ? (
                        <div className="flex items-center gap-1 min-w-0">
                          {q.problemNumber != null && (
                            <span className="shrink-0 text-[11px] font-bold text-gray-400">#{q.problemNumber}</span>
                          )}
                          <span className="truncate text-xs font-semibold text-indigo-700">{q.problemTitle}</span>
                        </div>
      ...
      <div className="bg-white border-b border-gray-200 px-8 py-3.5 flex items-center justify-between sticky top-0 z-30 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-indigo-600 transition-colors font-medium"
          >
            <UserCog className="w-4 h-4" /> 비밀번호 변경
          </button>
          <button
            ...
            {pwDone ? (
              ...
              <form onSubmit={handlePwChange} className="flex flex-col gap-4">
                ...
                <button type="submit" disabled={pwLoading}
                  className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-xl transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
