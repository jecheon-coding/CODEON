    ...
    <div className="h-screen flex flex-col overflow-hidden bg-white">
      ...
      <div ref={containerRef} className="flex-1 flex min-h-0 overflow-hidden" style={{ userSelect: isDragging || isVDragging ? "none" : undefined }}>
        ...
        <div ref={rightPanelRef} className="flex-1 flex flex-col min-h-0 min-w-0 overflow-hidden" style={{ background: bg.editor }}>
          ...
          <div className="h-8 flex items-center justify-between px-3 shrink-0" style={{ background: bg.panel, borderBottom: `1px solid ${editorBorderColor}` }}>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1.5">
          ...
          {/* 상태바 */}
          <div className="h-4 flex items-center justify-between px-3 text-[9px] font-mono shrink-0"
          ...
          {/* 컨트롤 바 */}
          <div className="h-9 flex items-center justify-end px-3 shrink-0 gap-2"
          ...
          >
            {running || waitingInput ? (
              <button onClick={stopRun}
                className="flex items-center gap-1 px-3 py-1 bg-[#dc2626] text-white text-xs font-semibold rounded-md hover:bg-[#b91c1c] transition-colors"
              >
                <svg className="w-2.5 h-2.5" viewBox="0 0 16 16" fill="currentColor"><rect x="3" y="3" width="10" height="10" rx="1"/></svg>
                정지
              </button>
            ) : (
              <button onClick={handleRun} disabled={submitting}
                className="flex items-center gap-1 px-3 py-1 bg-[#639922] text-white text-xs font-semibold rounded-md hover:bg-[#52821a] transition-colors disabled:opacity-50"
              ><SvgPlay /> 실행</button>
            )}
            <button onClick={handleSubmit} disabled={running || waitingInput || submitting}
              className="flex items-center gap-1 px-3 py-1 bg-[#534AB7] text-white text-xs font-semibold rounded-md hover:bg-[#443da0] transition-colors disabled:opacity-50"
