...
import { buildStringJoinVisualization } from "@/lib/stringMethodsTraversal"
...
export default function StringJoinVisualizer({ code }: { code: string }) {
  const result = useMemo(() => buildStringJoinVisualization(code), [code])
  ...
  if (!result.ok) {
    return (
      <div className="not-prose my-3 flex items-start gap-2 px-4 py-3 bg-red-50 border border-red-200 rounded-xl text-xs text-red-700">
        ...
        <span>문자열 결합 데이터를 표시할 수 없습니다: {result.error}</span>
      </div>
    )
  }
  ...
  return (
    <div className="not-prose my-4 max-w-2xl mx-auto border border-gray-200 rounded-xl overflow-hidden bg-white">
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-gray-100 bg-gray-50">
        ...
        <span className="text-sm font-bold text-gray-800">{spec.title || "join()"}</span>
      </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <div>
          <span className="text-[10px] font-bold text-gray-400">결과 문자열</span>
